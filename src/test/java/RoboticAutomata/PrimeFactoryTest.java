...
public class PrimeFactoryTest
{
    ...
    {
		List<Integer> primes = 
				new ArrayList<Integer>(Arrays.asList(2, 3, 7, 11));
        Assert.assertEquals(primes,
        		PrimeFactory.stripNonPrimes(primes));
    }
	
	@Test(dependsOnMethods = "isPrimeTest")
    public void stripNonPrimesNegativeTest()
    {
		List<Integer> nonPrimes = 
				new ArrayList<Integer>(Arrays.asList(4, 10, 12, 33));
		List<Integer> expected = new ArrayList<Integer>();
        Assert.assertEquals(expected,
        		PrimeFactory.stripNonPrimes(nonPrimes));
    }
